...
public class Calculate_L0_Solid {
	...
	public static double calculate_P1A1_L0() throws SQLException, ClassNotFoundException, IOException {
		...
		// get product name id from product table
		...
		while (getprodname_id.next()) {
			...
			ResultSet prod_basis_relation_id = stmt.executeQuery("SELECT * FROM edocs.product_basis_of_calculation_relation where product_id='" + prodname_id + "'");
				...
				while (basisOfcalc.next()) {
					...
					if (dose_based_flag == 1 && health_based_flag == 1) {
						...
						while (getname_id.next()) {
							...
							ResultSet basis_relation_id = stmt.executeQuery("SELECT * FROM edocs.product_basis_of_calculation_relation where product_id='"+ name_id + "'");
							...
							while (dosebaseddata.next()) {
								...
								while (productdata.next()) {
									...
									if (Dose_of_active == 0) { // if dose of active is null
										System.out.println("sf: " + Safety_Factor);
										System.out.println("active Con: " + Active_Concen);
										System.out.println("Product Dose: " + Product_Dose);
										System.out.println("min dose: " + min_no_of_dose);
						...
						while (getname_id.next()) {
							...
							while (dosebaseddata.next()) {
								...
								while (productdata.next()) {
									...
									System.out.println("Use Dose L0:" + L0); // getting dose L0 here
									return L0; // getting lowest L0 b/w 2
								} // closing 1st while
							} // closing 2st while
						} // closing 3st while
